...
from app.core.deps import get_chunk_repository, get_dataset_store, get_document_repository, get_eval_store, get_llm_client, get_vector_store
...
from app.services.datasets import EvalDatasetStore
...
def reprocess_document(document_id: str, request: Request, repo: DocumentRepository = Depends(get_document_repository)) -> ApiResponse:
    doc = repo.get(document_id)
    if not doc:
        return ApiResponse(success=False, error="NOT_FOUND", data=None, trace_id=_tid(request))
    doc.status = "reprocessing"
    return ApiResponse(data={"id": doc.id, "status": doc.status}, trace_id=_tid(request))
...
@router.post("/playground/chunk/commit", response_model=ApiResponse)
def chunk_commit(payload: dict, request: Request) -> ApiResponse:
    text = payload.get("text", "")
    size = int(payload.get("size", 200))
    overlap = int(payload.get("overlap", 20))
    chunks = fixed_chunk(text, size=size, overlap=overlap)
    return ApiResponse(data={"committed": True, "chunk_count": len(chunks), "config": {"size": size, "overlap": overlap}}, trace_id=_tid(request))


@router.get("/embedding/profiles", response_model=ApiResponse)
def embedding_profiles(request: Request) -> ApiResponse:
    profiles = [
        {"name": "default", "dimension": 8, "provider": "deterministic-stub"},
        {"name": "high-recall", "dimension": 8, "provider": "deterministic-stub"},
    ]
    return ApiResponse(data={"items": profiles}, trace_id=_tid(request))


@router.post("/playground/embed/commit", response_model=ApiResponse)
def embed_commit(payload: dict, request: Request) -> ApiResponse:
    text = payload.get("text", "")
    profile = payload.get("profile", "default")
    vector = embed_text(text)
    return ApiResponse(data={"committed": True, "profile": profile, "dimension": len(vector)}, trace_id=_tid(request))


@router.post("/evaluations/datasets", response_model=ApiResponse)
def create_eval_dataset(payload: dict, request: Request, dataset_store: EvalDatasetStore = Depends(get_dataset_store)) -> ApiResponse:
    name = payload.get("name", "default-dataset")
    items = payload.get("items", [])
    dataset_id = payload.get("dataset_id")
    ds = dataset_store.create(name=name, items=items, dataset_id=dataset_id)
    return ApiResponse(data=ds.__dict__, trace_id=_tid(request))


@router.get("/evaluations/datasets", response_model=ApiResponse)
def list_eval_datasets(request: Request, dataset_store: EvalDatasetStore = Depends(get_dataset_store)) -> ApiResponse:
    return ApiResponse(data={"items": dataset_store.list()}, trace_id=_tid(request))


@router.get("/evaluations/runs/{run_id}/results", response_model=ApiResponse)
def get_evaluation_run_results(run_id: str, request: Request, eval_store: EvalRunStore = Depends(get_eval_store)) -> ApiResponse:
    row = eval_store.get(run_id)
    if not row:
        return ApiResponse(success=False, error="NOT_FOUND", data=None, trace_id=_tid(request))
    metrics = row.get("metrics", {})
    items = [{"metric_name": k, "metric_value": v} for k, v in metrics.items() if isinstance(v, (int, float))]
    return ApiResponse(data={"run_id": run_id, "items": items}, trace_id=_tid(request))


@router.get("/quality/live", response_model=ApiResponse)
def get_quality_live(request: Request, run_id: str | None = None, eval_store: EvalRunStore = Depends(get_eval_store)) -> ApiResponse:
    row = eval_store.get(run_id) if run_id else None
    metrics = row.get("metrics", {}) if row else {}
    return ApiResponse(data={"run_id": run_id, "latency_p95_ms": metrics.get("latency_p95_ms"), "token_cost": metrics.get("token_cost"), "retrieval_hit_ratio": metrics.get("retrieval_hit_ratio")}, trace_id=_tid(request))
