...
from app.services.datasets import EvalDatasetStore, InMemoryEvalDatasetStore, PostgresEvalDatasetStore, SQLiteEvalDatasetStore
...
_dataset_store_singleton: EvalDatasetStore | None = None
...
def get_dataset_store() -> EvalDatasetStore:
    global _dataset_store_singleton
    if _dataset_store_singleton is None:
        backend = os.getenv("RACHEL_EVAL_BACKEND", "memory")
        if backend == "postgres":
            _dataset_store_singleton = PostgresEvalDatasetStore()
        elif backend == "sqlite":
            _dataset_store_singleton = SQLiteEvalDatasetStore()
        else:
            _dataset_store_singleton = InMemoryEvalDatasetStore()
